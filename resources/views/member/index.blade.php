...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        ...
        {{-- <a href="{{ route('member.create') }}"
            class="inline-flex items-center gap-1.5 px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-[12.5px] font-semibold rounded-lg transition">
    ...
    <div class="card">
        ...
        <div class="p-4 border-b border-gray-100">
            <form method="GET" action="{{ route('member.index') }}" class="flex flex-col sm:flex-row gap-2">
                <div class="relative flex-1">
                    <i
                        class="fa-solid fa-magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 text-[11px] text-gray-400"></i>
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Cari nama atau kode member…" class="filter-input w-full pl-9">
                </div>
                ...
                <tbody>
                    @forelse($members as $member)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="font-mono text-[11.5px] text-gray-500 bg-gray-100 px-2 py-1 rounded-md border border-gray-200">
                                            {{ $member->kode_member }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="text-center">
                                        @php
                                            // Cek secara real-time di tampilan
                                            $isExpiredByDate = $member->tanggal_kadaluarsa && \Carbon\Carbon::parse($member->tanggal_kadaluarsa)->endOfDay()->isPast();
                                        @endphp

                                        @if($member->status === 'expired' || $isExpiredByDate)
                                            {{-- Jika status di DB sudah expired ATAU tanggalnya memang sudah lewat --}}
                                    ...
                                    <td>
                                        <div class="flex items-center justify-center gap-1">
                                            {{-- Detail ── --}}
                                            <a href="{{ route('member.show', $member->id) }}" class="act-btn" title="Lihat Detail">
                                                <i class="fa-solid fa-eye text-[11px]"></i>
                                            </a>

                                            {{-- Toggle Status ── --}}
                                            <form method="POST" action="{{ route('member.toggle', $member->id) }}"
                                                id="form-toggle-{{ $member->id }}">
                                                @csrf @method('PATCH')
                                                <button type="button" onclick="Swal.confirm({
                            title: 'Ubah Status?',
                            text: 'Member ini akan menjadi {{ $member->status === 'aktif' ? 'Nonaktif' : 'Aktif kembali' }}.',
                            confirmText: 'Ya, Ubah',
                            onConfirm: () => document.getElementById('form-toggle-{{ $member->id }}').submit()
                        })" class="act-btn success" title="{{ $member->status === 'aktif' ? 'Nonaktifkan' : 'Aktifkan' }}">
                                                    <i
                                                        class="fa-solid {{ $member->status === 'aktif' ? 'fa-toggle-on' : 'fa-toggle-off' }} text-[13px]"></i>
                                                </button>
                                            </form>
                                        </div>
                                    </td>
        ...
        @if($members->hasPages())
            <div class="px-4 py-3 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-3">
                <p class="text-[12px] text-gray-400">
                    Menampilkan <span
                        class="font-semibold text-gray-600">{{ $members->firstItem() }}–{{ $members->lastItem() }}</span>

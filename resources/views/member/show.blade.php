...
@section('content')
    ...
    <div class="mb-4">
        <a href="{{ route('member.index') }}"
            class="inline-flex items-center gap-2 text-[13px] text-gray-500 hover:text-gray-700 transition">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="space-y-4">
            ...
            <div
                class="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-2xl p-6 text-white shadow-lg shadow-emerald-200 relative overflow-hidden">
                {{-- Watermark Icon --}}
            ...
            <div class="grid grid-cols-1 gap-2">
                ...
                {{-- Ganti action sementara di member.show untuk tes --}}
                <form method="POST" action="{{ route('member.toggle', $member->id) }}" id="form-toggle-show">
                    @csrf @method('PATCH')
                    <button type="button" onclick="Swal.confirm({
                title: '{{ $member->status === 'aktif' ? 'Nonaktifkan Member?' : 'Aktifkan Member?' }}',
                text: 'Akses member ini ke sistem akan {{ $member->status === 'aktif' ? 'ditutup sementara' : 'dibuka kembali' }}.',
                confirmText: 'Ya, Lanjutkan',
                onConfirm: () => document.getElementById('form-toggle-show').submit()
            })" class="w-full py-2.5 text-[13px] {{ $member->status === 'aktif' ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600' }} text-white rounded-lg font-medium transition flex items-center justify-center gap-2">
                        <i class="fa-solid {{ $member->status === 'aktif' ? 'fa-xmark' : 'fa-check' }} text-[11px]"></i>
                        {{ $member->status === 'aktif' ? 'Nonaktifkan Member' : 'Aktifkan Kembali' }}
                    </button>
                </form>
            </div>
        </div>
                        ...
                        <div class="flex items-center justify-between py-3">
                            <div>
                                ...
                                <div class="text-[11px] text-gray-400 mt-0.5">
                                    {{ \Carbon\Carbon::parse($trx->tanggal_pembayaran ?? $trx->created_at)->format('d M Y') }} ·
                                    {{ $trx->paket->nama_paket ?? '-' }}
                                </div>
                            </div>
                            <div class="text-right">
                                <div class="text-[13px] font-semibold text-gray-800">Rp {{ number_format($trx->jumlah_bayar) }}
                                </div>
                                ...
                                @else
                                    <span
                                        class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">{{ strtoupper($trx->status) }}</span>
                                @endif
                            </div>
                        </div>
                    @empty
